...
def sentiment(finalResult):
    ...
    if (sentiment.score > 0.05):
        mqtt_client.publish("Sentiment","{\"sentiment\": \"Positive\"}")
    elif (sentiment.score < -0.05):
        mqtt_client.publish("Sentiment","{\"sentiment\": \"Negative\"}")
    else:
        mqtt_client.publish("Sentiment","{\"sentiment\": \"Neutral\"}")
##    
